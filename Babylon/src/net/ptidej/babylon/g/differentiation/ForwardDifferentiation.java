...
import static jdk.incubator.code.op.CoreOp.invoke;
import static jdk.incubator.code.op.CoreOp.mul;
import static jdk.incubator.code.op.CoreOp.neg;
...
public final class ForwardDifferentiation {
	...
	FuncOp partialDiff() {
		...
		FuncOp dfcm = fcm.transform(String.format("d%s_darg%d", fcm.funcName(), indI), (block, op) -> {
	...
	Value diffOp(Block.Builder block, Op op) {
		...
		case CoreOp.MulOp useless -> {
			...
			yield block.op(add(block.op(mul(dlhs, outputRhs)), block.op(mul(outputLhs, drhs))));
		}
		case CoreOp.ConstantOp useless -> {
			// Copy input operation
			block.op(op);
	...
	void adaptSuccessor(CopyContext cc, Block.Reference from) {
		List<Value> as = from.arguments().stream().filter(activeSet::contains).toList();
		if (!as.isEmpty()) {
